...
##                    print('OPTIONS: loaded hardcoded list')
                ...
                self.elements['CHOICE_T'].configure(bd = 1, justify = tk.LEFT, anchor = 'w', font = ("Helvetica", "13"),
                    padx = 0, pady = 0, relief = tk.RAISED, text = ' These are your translation options:',)
                ...
                self.elements['CHOICE_T'].menu.configure(tearoff = 0, bd = 0, font = ("Helvetica", "13"))
...
##                    print('OPTIONS_T: loaded hardcoded list')
    ...
    def id_changed(self, e = None):
        ...
        if self.options_known:
            ...
            self.selection.set('Default: de, en')
            self.draw_language_elements()
    ...
    def fetch_languages(self, e = None):
        ...
        try:
            ...
            ## This is a stomach-turning hack... TranscriptList comes from the API and it's only __function__ is __iter__, so I had to make do!
            if len(list(iter(self.transcript_list))) == 0: raise IOError('Zero transcripts available.')
        except IOError as e:
            if DEBUG: print('FETCH: ', e)
            self.error.set('Zero transcripts found for this video.')
            return
